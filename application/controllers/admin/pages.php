...
class Pages extends MY_Controller
{
	...
	{
		$result_set 			 = 	$this->db_handler->get('tbl_pages');
		$this->data['listData'] = 	$result_set;
		$this->template->load('main','admin/pages/pages_listing',$this->data);
	}
	...
	public function save($update_id="")
	{
		if(!empty($update_id))
		{
			$update_data	=	$this->db_handler->get('tbl_pages','id',$update_id);
		...
		{	
			...
			{					
				if($insert_id = $this->db_handler->save('tbl_pages',$page_data))
				{					
					if($page_data['id']>0)
					{
		...
		$this->template->load('main','admin/pages/add_page',$this->data);
	}
	...
	public function delete($id)
	{
		if(!empty($id))
		{
			if($this->db_handler->delete('tbl_pages','id',$id))
