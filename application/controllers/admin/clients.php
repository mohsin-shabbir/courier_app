...
class Clients extends MY_Controller
{
	...
	public function save($update_id="")
	{
		...
		{	
			...
			if(!empty($client_data['country_code']) && (strcmp($client_data['country_code'],'GB')==0))
			...
			{	
				...
				$client_data['created_by']	=	"ADMIN";
				$client_data['updated_by']	=	"ADMIN";
				$client_data['updater_id']	=	"1";
				...
				if($insert_id=$this->db_handler->save('clients',$client_data))
				{
					...
					if($client_data['id']>0)
					{
						$this->session->set_flashdata('success_message','Client updated successfully   '.(!empty($image_message)?$image_message:''). "   ".(!empty($thumb_message)?$thumb_message:''));
					}
					else
					{
						$this->session->set_flashdata('success_message','Client created successfully   '.(!empty($image_message)?$image_message:''). "   ".(!empty($thumb_message)?$thumb_message:''));
					}
	...
	public function list_datatable()
	{
		...
		{
			$country_code	=	$this->db_handler->get('countries',array('name'=>$paged_data['sSearch_6']));
			if(!empty($country_code[0]))
			{
				...
				if(!empty($country_code['code']))
				{
					...
					$where['clients.country_code']=$country_code;
					$client_deleted = TRUE;
				}
			}
		}
